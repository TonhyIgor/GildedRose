...
public class ItemBackstage implements ManejoItems {
    ...
    public static final int DIAS_MAXIMOS_PARA_VENTA = 0;
    ...
    public void actualizarItems() {
        for (Item item : items) {
            if (item.quality <= CALIDAD_MAXIMA) {
                ...
                if (item.sellIn <= CANTIDAD_DIAS_INCREMENTO_DOBLE && item.sellIn > DIAS_MAXIMOS_PARA_VENTA) {
                    item.quality = item.quality + INCREMENTO_CALIDAD_TRIPLE;
                }
                if (item.sellIn == DIAS_MAXIMOS_PARA_VENTA) {
